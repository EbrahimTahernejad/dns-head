// Package srawtr is "striped RAW": opens N parallel RAW+smux sessions
...
// where one RAW flow is capped (per-flow shaping, tail-drop on a single
// queue, etc).
//
// Wire-level: each sub-stream is a regular smux stream over our RAW transport. On top
...
import (
	...
	_ "github.com/ebrahimkh/dns-head/transports/rawtr" // register raw so we can grab it
)

// NumLanes is N — the number of parallel RAW sessions per logical tunnel.
var NumLanes = func() int {
	if v := os.Getenv("DNSH_SRAW_LANES"); v != "" {
...
func (transport) Name() string { return "sraw" }

func init() { transports.Register(transport{}) }

// rawInner returns the registered "raw" transport (lazy to avoid init-order
// surprises).
func rawInner() transports.Transport {
	t, err := transports.Get("raw")
	if err != nil {
		panic("sraw: kcp transport not registered: " + err.Error())
	}
	return t
}

func (transport) Listen(addr string, cfg transports.Config) (transports.Listener, error) {
	// One UDP listener, rawtr demuxes by remote addr — so N clients with N
	...
	inner, err := rawInner().Listen(addr, cfg)
	if err != nil {
		return nil, err
	}
	l := &listener{
...
func (transport) Dial(ctx context.Context, serverAddr string, cfg transports.Config) (transports.Conn, error) {
	...
	for i := 0; i < NumLanes; i++ {
		c, err := rawInner().Dial(ctx, serverAddr, cfg)
		if err != nil {
			for j := 0; j < i; j++ {
				subs[j].Close()
			}
...
func (c *dialConn) AcceptStream(ctx context.Context) (transports.Stream, error) {
	...
	return nil, errors.New("sraw: AcceptStream not implemented on client side")
}
...
type pendingConn struct {
	subs    []transports.Conn // we collect underlying raw+smux conns by remote IP
...
func (a *acceptConn) OpenStream(ctx context.Context) (transports.Stream, error) {
	return nil, errors.New("sraw: server-initiated streams not supported")
}
...
func (l *listener) Accept(ctx context.Context) (transports.Conn, error) {
	...
	case <-ctx.Done():
		return nil, ctx.Err()
	case <-l.closed:
		return nil, errors.New("sraw: listener closed")
	}
}
